...
int main() {
	...
	do {
		...
		std::cout << "\nMenue:" << std::endl << "-----------------------------"
				...
				<< "(7): Beenden" << std::endl << "(8): Liste sortieren"
				<< std::endl;
		// *INDENT-ON*
		std::cin >> abfrage;
		...
		case '5': {
			...
			std::vector<Student>::iterator it = std::find_if(data.begin(),
					data.end(), find_student(matrikel));
			if (it != data.end()) {
				std::cout << "Loesche Studenten " << it->getName() << std::endl;
				data.erase(it);
...
void print_data(std::vector<Student> data, bool reverse) {
	if (reverse) {
		for (std::vector<Student>::reverse_iterator it = data.rbegin();
				it != data.rend(); it++) {
			std::cout << *it;
		}
	} else {
		for (std::vector<Student>::iterator it = data.begin(); it != data.end();
				it++) {
			std::cout << *it;
		}
	}
